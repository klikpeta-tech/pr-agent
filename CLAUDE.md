...
**A hung process isn't a dead process.** `entrypoint.sh`'s own self-healing (`wait_any` in the loop at the bottom) originally only detected a process *exiting* (`kill -0`); it had no way to notice one that's alive but wedged — stuck on a slow or hanging upstream call, for example. That happened for real on 2026-09-24: pr-agent stopped answering any request at all while Fly still reported the machine as "started", and nothing restarted it until a manual `fly apps restart`. `fly.pr-agent.toml` has an `[[http_service.checks]]` block (`GET /`, which auto-approve-proxy forwards straight through to pr-agent on :3001, so the check exercises the same path real traffic takes) — but Fly's own health checks only stop *routing* to an unhealthy machine, they never restart or stop one automatically ("this needs to be done manually" per Fly's own docs). `wait_any` itself now also probes that same endpoint every 5s and, after `WATCHDOG_FAILURE_THRESHOLD` (default 6, ~30s) consecutive failures, tears down all three processes and exits so Fly restarts the container — the actual auto-recovery, with the `[[http_service.checks]]` block serving mainly to keep Fly's proxy from routing traffic to the machine during that ~30s window. The check subsystem is separate from the `http_service` idle-connection tracking that drives `auto_stop_machines`, so it shouldn't stop the app from suspending when genuinely idle — but that interaction is worth re-verifying (`fly status` should still show `suspended` after a few idle minutes, not stuck at `started`) if the check config ever changes.
...
- **pr-agent's `fallback_models` cannot do this job on its own.** OpenAI overage isn't refused, it's billed — so the primary model keeps returning 200 and no fallback ever fires. This is exactly why the proxy has to manufacture the "budget spent" signal itself for the `openai` bucket; pr-agent has no other way to see it. (`openai_mini` differs only in the reaction: the proxy's own 429 for an exhausted `openai_mini` bucket — same mechanism as `openai`'s, just with no in-proxy downgrade target — is the error that lets `fallback_models` move on to DeepSeek.)
